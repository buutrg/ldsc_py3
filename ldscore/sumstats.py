...
def _check_variance(log, M_annot, ref_ld):
    ...
    if ii.any():
        ...
        ii_snp = ref_ld.columns[1:][ii]
        ii_m = M_annot.loc[:, ii_snp].astype(bool)
        ii_m = ii_m.any(axis=0)
        M_annot = M_annot.loc[:, ~ii_m]
        ref_ld = ref_ld.loc[:, ~ii]
        ref_ld = ref_ld.loc[:, ~ii]  # NB there is a SNP column here
